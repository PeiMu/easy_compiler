...
public:
  explicit FuncAST(std::string name,
...
class CallAST final : public StmtAST {
public:
  explicit CallAST(std::unique_ptr<ExprAST> result,
                   std::unique_ptr<FuncAST> callee,
                   std::vector<std::unique_ptr<ExprAST>> param)
      : result_(std::move(result)), callee_(std::move(callee)),
